...
class Corpus( object ):
    ...
    def tokenize( self, path ):
        ...
        with open( path, 'r' ) as f:
            ...
            lengths = []
            labels = torch.LongTensor( count,  NUM_TASKS).fill_( 0 )

            for idx, record in enumerate( Reader ):
                labels[ idx, 0 ] = int( record[ 1 ] )  # protest/non-protest
                labels[ idx, 1 ] = int( record[ 2 ] )  # ev_type
                labels[ idx, 2 ] = int( record[ 3 ] )  # pop_type
